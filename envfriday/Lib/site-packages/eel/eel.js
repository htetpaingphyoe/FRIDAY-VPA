eel = {
    ...
    _mock_py_functions: function() {
        if (!Array.isArray(eel._py_functions)) {
            eel._py_functions = []; // Ensure it's at least an empty array
        }
    ...
    _position_window: function(page) {
        if (!eel._start_geometry || !eel._start_geometry['default']) {
            console.warn("Warning: _start_geometry is not defined. Skipping window positioning.");
            return;
        }
